...
class CsvToJsonTab(ttk.Frame):
    ...
    def setup_frames(self):
        ...
        self.headers_canvas = tk.Canvas(self.header_config_frame, bg=COLOR_BG_APP, highlightthickness=0)
        ...
        self.headers_frame = tk.Frame(self.headers_canvas, bg=COLOR_BG_APP)
        self.headers_canvas.window(
        ...
        self.raw_json_text = tk.Text(raw_frame, wrap=tk.WORD, font=("Consolas", 10),
                                     bg=COLOR_BG_SURFACE, fg=COLOR_TEXT_MAIN, insertbackground=COLOR_TEXT_MAIN)
    ...
    def create_widgets(self):
        ...
        tk.Label(self.top_frame, text="Input CSV File:", bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN).grid(
        ...
        self.csv_browse_button = ttk.Button(
            self.top_frame, text="Browse...", command=self.load_csv_file
        )
        self.csv_browse_button.grid(row=0, column=2, padx=5, pady=2)

        tk.Label(self.top_frame, text="Output JSON File:", bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN).grid(
        ...
        self.json_browse_button = ttk.Button(
    ...
    def load_headers(self):
        ...
        try:
            ...
            tk.Label(
                self.headers_frame, text="JSON Key Name", font=("Arial", 10, "bold"), bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN
            ).grid(row=0, column=0, padx=5, pady=2)
            tk.Label(self.headers_frame, text="Role", font=("Arial", 10, "bold"), bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN).grid(
                row=0, column=1, padx=5, pady=2
            )
            tk.Label(
                self.headers_frame, text="Nested Under", font=("Arial", 10, "bold"), bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN
            ).grid(row=0, column=2, padx=5, pady=2)
            tk.Label(
                self.headers_frame,
                text="Part Name (e.g., 'contents')",
                font=("Arial", 10, "bold"), bg=COLOR_BG_APP, fg=COLOR_TEXT_MAIN
            ).grid(row=0, column=3, padx=5, pady=2)

            for i, header in enumerate(self.headers):
                row_num = i + 1

                header_entry = tk.Entry(self.headers_frame, width=20, bg=COLOR_BG_SURFACE, fg=COLOR_TEXT_MAIN, insertbackground=COLOR_TEXT_MAIN)
                ...
                part_name_entry = tk.Entry(self.headers_frame, width=25, bg=COLOR_BG_SURFACE, fg=COLOR_TEXT_MAIN, insertbackground=COLOR_TEXT_MAIN)
                ...
                def toggle_widgets(event, p_entry=part_name_entry, r_dropdown=role_dropdown):
                    role = r_dropdown.get()
                    if role in ["Hierarchical Key", "Key Name and Value"]:
                        p_entry["state"] = "normal"
                    else:
    ...
    def update_output_with_json(self, data):
        ...
        try:
            formatted_json = orjson.dumps(data, option=orjson.OPT_INDENT_2).decode()
